...
A simple exclusion of all rows with incomplete data results in exclusion of `r nrow(df)-nrow(df_clean)` rows, or `r round(100*(1-nrow(df_clean)/nrow(df)))`% of observations - a considerable loss of data.

Instead, an imputation of missing data using the predictive mean matching approach implemented in the **mice** package is applied.    
...
```{r}
plot_histogram(log(df$TARGET_AMT))
```
...
```{r}
df_eval = read_csv("insurance-evaluation-data.csv")
...
# Plot scatter for the first 5 features

featurePlot(x = df_viz$continuous[2:6], 
            y =df$TARGET_FLAG,
            plot = "pairs",
            auto.key = list(columns = 2),
            par.settings = list(superpose.symbol = list(pch = 1, cex = 0.1,
                                                   col = c("black", "red")))
            )
```

```{r}
# Plot scatter for the next 5 features

featurePlot(x = df_viz$continuous[7:11], 
            y =df$TARGET_FLAG,
            plot = "pairs",
            auto.key = list(columns = 2),
            par.settings = list(superpose.symbol = list(pch = 1, cex = 0.1,
...
featurePlot(x = df_viz$continuous[12:15], 
            y =df$TARGET_FLAG,
            plot = "pairs",
            auto.key = list(columns = 2),
            par.settings = list(superpose.symbol = list(pch = 1, cex = 0.1,
